...
export const EmployeeRegistration: React.FC<EmployeeRegistrationProps> = ({ onRefreshAll, mode = 'REGISTRATION', currentSiteId }) => {
  ...
  const [adminTab, setAdminTab] = useState<'REQUESTS' | 'EMPLOYEES'>('REQUESTS');
  ...
  // Employee Whitelist state, moved here from Inventory Master per
  // explicit request — this tab now lives inside Vehicle Approval
  // Queue (renamed Employee & Vehicle Approval) instead.
  ...
  const [isEmpBulkModalOpen, setIsEmpBulkModalOpen] = useState(false);
  const [empBulkCsvText, setEmpBulkCsvText] = useState('');
  const [parsedBulkEmployees, setParsedBulkEmployees] = useState<Partial<Employee>[]>([]);
  const [empBulkUploading, setEmpBulkUploading] = useState(false);
  const [bulkSuccessMsg, setBulkSuccessMsg] = useState<string | null>(null);
  const fileInputRef = useRef<HTMLInputElement | null>(null);

  // Employee Whitelist filters/pagination
  ...
  const refreshData = async () => {
    setLoading(true);
    await Promise.all([fetchRegistrations(), fetchDomains(), fetchSlots(), fetchEmployees()]);
    setLoading(false);
  };
  ...
  // Employee Whitelist badge helper and handlers, moved from Inventory Master
...
EMP-3004,Siddharth Verma,Finance & Legal,Senior Analyst,+91 9876543213,siddharth@company.com,DL-01-EX-7711,HATCHBACK,Hyundai,Active`;
  ...
  const handleConfirmEmpBulkUpload = async () => {
    ...
    <div className="space-[#121826] space-y-6">
      ...
      {activeView === 'ADMIN_APPROVAL' && (
        <div className="space-y-6">
          ...
          {/* Sub-tabs: Registration Requests vs Employee Whitelist (moved
              here from Inventory Master per explicit request) */}
          <div className="flex gap-1 border-b border-slate-200">
            <button
              onClick={() => setAdminTab('REQUESTS')}
              className={`px-4 py-2 text-xs font-bold border-b-2 transition-colors ${adminTab === 'REQUESTS' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
            >
              Registration Requests
            </button>
            <button
              onClick={() => setAdminTab('EMPLOYEES')}
              className={`px-4 py-2 text-xs font-bold border-b-2 transition-colors ${adminTab === 'EMPLOYEES' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
            >
              Employee Whitelist
            </button>
          </div>
      ...
      {adminTab === 'EMPLOYEES' && (
          ...
          <div className="p-4 border-b border-slate-200 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs">
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input
                  type="text"
                  placeholder="Search employee ID, plate, name..."
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between text-xs text-slate-600 font-mono">
          <span>
            Showing Page {currentPage} of {totalEmpPages} (
            {filteredEmployees.length} total entries)
          </span>
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              onClick={() =>
                setCurrentPage((p) => Math.min(totalEmpPages, p + 1))
              }
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white border border-slate-200 rounded-3xl shadow-2xl w-full max-w-3xl overflow-hidden my-8">
            ...
            <div className="p-5 bg-slate-900 text-white flex items-center justify-between">
              ...
              <button
                onClick={() => setIsEmpBulkModalOpen(false)}
                className="p-1.5 bg-slate-800 hover:bg-slate-700 rounded-xl text-slate-400 hover:text-white transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            ...
            <div className="p-6 space-y-5 text-xs">
              ...
              <div className="pt-4 border-t border-slate-200 flex items-center justify-between">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <button
                    onClick={handleConfirmEmpBulkUpload}
                    disabled={empBulkUploading || parsedBulkEmployees.length === 0}
                  ...
                  >
                    {empBulkUploading && <RefreshCw className="w-4 h-4 animate-spin" />}
